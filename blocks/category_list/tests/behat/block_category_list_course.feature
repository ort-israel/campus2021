@block @block_category_list
Feature: Enable the category_list block on a course page and view it's contents
  ...
  Scenario: View the course list block on course page with hide all courses link enabled
    Given the following config values are set as admin:
      | block_category_list_hideallcourseslink | 1 |
  ...
  Scenario: View the course list block on course page with admin sees own course enabled
    Given the following config values are set as admin:
      | block_category_list_adminview | own |
    And the following "course enrolments" exist:
      | user | course | role           |
