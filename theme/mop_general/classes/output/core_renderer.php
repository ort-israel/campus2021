...
class core_renderer extends \theme_boost\output\core_renderer {
    ...
    public function standard_head_html() {
        ...
        $output .= "<link href=\"https://fonts.googleapis.com/css?family=Heebo:400,700,800&amp;subset=hebrew\" rel=\"stylesheet\">\n";
        return $output;
    }
    ...
    public function full_header() {
        ...
        return $this->render_from_template('theme_mop_general/header', $header);
    }
    ...
    public function headerimage() {
        ...
        $courseimage = $this->get_image_from_area_files();
    ...
    public function fordson_custom_menu() {
        ...
        if (isloggedin() && !isguestuser() && $hasdisplaymycourses) {
            ...
            $branchtitle = get_string('mycourses', 'theme_mop_general');
            $thisbranchtitle = get_string('thismycourses', 'theme_mop_general');
            $homebranchtitle = get_string('homemycourses', 'theme_mop_general');
            ...
            if ($courses = enrol_get_my_courses(NULL, 'fullname ASC')) {
                ...
                $nomycourses = '<div class="alert alert-info alert-block">' . get_string('nomycourses', 'theme_mop_general') . '</div>';
                if ($courses) {
                ...
                $noenrolments = get_string('noenrolments', 'theme_mop_general');
    ...
    protected function render_courseactivities_menu(custom_menu $menu) {
        ...
        foreach ($menu->get_children() as $item) {
            $context = $item->export_for_template($this);
            $content .= $this->render_from_template(' theme_mop_general/activitygroups', $context);
        }
        return $content;
    }
    ...
    public function courseactivities_menu() {
        ...
        $context = $this->page->context;
        if (isset($COURSE->id) && $COURSE->id > 1) {
            $branchtitle = get_string('courseactivities', 'theme_mop_general');
            $branchlabel = $branchtitle;
            $branchurl = new moodle_url('#');
            $branch = $menu->add($branchlabel, $branchurl, $branchtitle, 10002);
            $data = theme_mop_general_get_course_activities();
    ...
    public function social_icons() {
        ...
        return $this->render_from_template(' theme_mop_general/socialicons', $socialcontext);
    }
    ...
    public function customlogin() {
        ...
        return $this->render_from_template(' theme_mop_general/customlogin', $customlogin_context);
    }
    ...
    public function teacherdash() {
        ...
        if (isloggedin() && isset($COURSE->id) && $COURSE->id > 1) {
            ...
            if (has_capability('moodle/course:viewhiddenactivities', $context)) {
                $togglebutton = get_string('coursemanagementbutton', 'theme_mop_general');
            } else {
                $togglebuttonstudent = get_string('studentdashbutton', 'theme_mop_general');
            }
        }
        $course = $this->page->course;
        $context = context_course::instance($course->id);
        ...
        $userlinks = get_string('userlinks', 'theme_mop_general');
        $userlinksdesc = get_string('userlinks_desc', 'theme_mop_general');
        $qbank = get_string('qbank', 'theme_mop_general');
        $qbankdesc = get_string('qbank_desc', 'theme_mop_general');
        $badges = get_string('badges', 'theme_mop_general');
        $badgesdesc = get_string('badges_desc', 'theme_mop_general');
        $coursemanage = get_string('coursemanage', 'theme_mop_general');
        $coursemanagedesc = get_string('coursemanage_desc', 'theme_mop_general');
        ...
        $progress = $this->render_from_template(' theme_mop_general/progress-bar', $progresschartcontext);

        $gradeslinkstudent = new moodle_url('/grade/report/user/index.php', array(
            'id' => $PAGE->course->id
        ));
        $hascourseinfogroup = array(
            'title' => get_string('courseinfo', 'theme_mop_general'),
            'icon' => 'map'
        );
        ...
        $hascoursestaff = array(
            'title' => get_string('coursestaff', 'theme_mop_general'),
            'icon' => 'users'
        );
        ...
        $activitylinkstitle = get_string('activitylinkstitle', 'theme_mop_general');
        $activitylinkstitle_desc = get_string('activitylinkstitle_desc', 'theme_mop_general');
        $mygradestext = get_string('mygradestext', 'theme_mop_general');
     ...
     * @throws \coding_exception
     */
    private function get_image_from_area_files() {
        global $CFG, $COURSE;
        require_once($CFG->libdir . '/filestorage/file_storage.php');
        require_once($CFG->dirroot . '/course/lib.php');
